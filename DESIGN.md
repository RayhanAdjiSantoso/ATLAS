...
Business Overview keeps brand/period controls and channel navigation above the analysis. Each channel uses a 192px text domain rail and `minmax(0, 1fr)` canvas separated by 28px. The rail sticks 24px from the viewport top. The canvas has a flat KPI band and an open analysis panel, separated by 28px. Business Growth places its three exact metrics in a divided open row, then a full-width GMV plot and two supporting charts. GMV measures its available width so the plot fills the panel. Executive Snapshot is a separate cross-channel view with its performance table, charts and Minutes of Meeting. Meta and TikTok retain importer-pending states without fabricated metrics.
...
Channel breakpoints: **1200px** — rail becomes 160px, gutter 20px, KPIs two columns. **960px** — domain navigation moves above the canvas in three columns. **640px** — navigation becomes two columns; single-period metrics and charts stack, while comparison metrics remain paired. At a **720px analysis-container width**, funnel layouts and generic period comparisons stack with explicit period labels. Product and other auto-fit tracks clamp their minimum to the available width. The application drawer and Minutes of Meeting retain their existing responsive behavior.
...
### Named Rules
**The Hairline Rule.** Structure is drawn with 1px greys, not with weight, tint blocks, or coloured bars. The channel KPI band uses open cells with bottom hairlines, no enclosing border or shadow, because the eight metrics are one reading of one period.
...
- **Console domain rail** (design-system tokens, Inter): purely typographic, **no icons at all**. Rows are 0.82rem/550 muted text with a 7px state dot; active is white on a **square-edged, flush navy** band, sliding between rows on a framer-motion spring (`stiffness 520, damping 44, mass .6`).
...
One shared record has two deliberately different readings, and neither invents its own controls. Date is `DatePicker` (DateRangePicker's popover shell, preset column and exported `MonthCalendar`, at one month's width); meeting type and every short choice list is `SelectMenu` (the brand picker's trigger states and result rows, portalled so clipping cards cannot cut it off, with a checkbox `multiple` mode). Both live in `components/common/fieldControls.css`.

In Brand Settings, the composer is a minimisable band: a `.brand-ds-name`-style bar (chevron, icon tile, label pair, "Belum disimpan" chip) over a two-column field grid that copies `.brand-field` exactly — date and type, full-width recap, then MIL and Client task areas whose label counts parsed tasks and PICs as you type — closed by `.brand-save-bar`. Validation and save failures render inline under the grid, not in the page flash. Saved meetings are grouped under month rules and minimise like `.brand-ds` dataset rows: date tile, long date + type, one-line recap preview, a `.brand-ds-status` task chip, `.brand-ds-fileacts` Edit/Hapus; opened, the full recap and read-only PIC task lists with completion state. A type filter reuses `.brand-status-filter` chips with counts; "Buka semua / Tutup semua" toggles every row. Editing scrolls the composer into view and marks the row with a 3px accent inset; unsaved changes are confirmed before being discarded.

In Business Overview, the brand's records feed one `.con-focus`-styled card after the executive table and charts — on the **Executive Snapshot** view only, never repeated above the other domains — minimisable from its head (remembered per session) with a one-line count of in-period meetings and open tasks when closed. The card loads the brand's whole record once; only the reading pane is filtered to the dashboard period. The reading pane holds the recap `SelectMenu` (all in-period records by default) and the Gemini action, then two bordered disclosures: the structured summary (150px label column, copy and PDF actions) and the full recap list. Changing the selection after generating marks the summary stale rather than deleting it. The right rail stacks **MIL before Client**, matching the internal-to-external handoff order, and is deliberately **not** bound to the period: a to-do stays owed until someone ticks it, so the rail gathers every open task from every meeting of the brand, each tagged with the date and type of the meeting it came from. When the period holds no meetings the reading pane says so and the rail still shows the backlog.

The task rail translates authored lines into a PIC hierarchy: a line ending in a colon becomes a PIC heading, following lines become indented tasks on a hairline branch, and unheaded lines fall under "Tanpa PIC" (`components/mom/momModel.js`, shared by both pages). PIC headings merge across meetings (case-insensitively) with a count chip; each ownership section shows active/done counts. Finished tasks from all meetings collect in one scrollable disclosure. Completing a task moves it into a nested "Sudah dilakukan" disclosure; its check control reverses the move. Completion is optimistic and sequenced per record — only the newest response settles the UI, and a failure rolls back to the last server-confirmed state with an inline "Coba lagi". A brand change reloads the record; a period change only re-scopes the recap selection and discards the summary, never the task rail.
...
- **Do** use `DatePicker` and `SelectMenu` for any new date or short choice field, instead of a native `<input type="date">` or `<select>` beside drawn controls.
...
### Executive Snapshot — comparison and revenue

The performance table has open headers, left-hand row-group labels, and the current period first. Its current revenue figures receive the strongest scale and accent. Relative magnitude tracks compare the same metric across recorded periods only when at least two nonnegative values exist. Missing values remain explicit. Mobile keeps a horizontally scrollable, keyboard-focusable table region. Daily revenue uses bounded horizontal Bézier controls between recorded days; missing days still break the curve. A peak caption exposes the highest recorded day. The snapshot donut uses exact butt-ended arcs so small channel shares are not overstated by rounded caps. A compact rupiah total sits inside the ring; the full amount stays below it and follows the hovered or keyboard-focused legend entry. The channel legend stacks below the ring on narrow screens. These treatments are scoped to Executive Snapshot; the existing shared donut remains the default elsewhere.
