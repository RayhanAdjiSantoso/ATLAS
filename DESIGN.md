...
In Business Overview, in-period records feed one `.con-focus`-styled card directly after the KPI strip, minimisable from its head (remembered per session) with a one-line count of meetings and active tasks when closed. The reading pane holds the recap `SelectMenu` (all in-period records by default) and the Gemini action, then two bordered disclosures: the structured summary (150px label column, copy and PDF actions) and the full recap list. Changing the selection after generating marks the summary stale rather than deleting it. The right rail stacks **MIL before Client**, matching the internal-to-external handoff order, reads the latest meeting by default and can switch source through a `SelectMenu`.

The task rail translates authored lines into a PIC hierarchy: a line ending in a colon becomes a PIC heading, following lines become indented tasks on a hairline branch, and unheaded lines fall under "Tanpa PIC" (`components/mom/momModel.js`, shared by both pages). Each ownership section shows active/done counts and a progress bar. Completing a task moves it into a nested "Sudah dilakukan" disclosure; its check control reverses the move. Completion is optimistic and sequenced per record — only the newest response settles the UI, and a failure rolls back to the last server-confirmed state with an inline "Coba lagi". Brand or period changes discard the previous summary, error and task source.
